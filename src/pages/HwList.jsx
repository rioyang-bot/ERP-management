...
import { Search, Columns3, Edit2, X, Server, User, MapPin, MoreHorizontal, Trash2, ShoppingBag, AlertTriangle, CheckCircle, Save, Monitor, Settings, ShieldAlert, Archive, RotateCcw, Cpu, Send, History, Building2, RefreshCw } from 'lucide-react';
...
const HwList = ({ isSplitMode = false }) => {
  ...
  const { isVisible, toggle: toggleColumn, showAll: showAllColumns, hiddenCount } = useColumnPreferences('hwListColumns', HW_COLUMNS);
                  ...
                  <div style={{ marginBottom: '8px', borderBottom: '1px solid var(--border-color)', paddingBottom: '6px' }}>
                    <div style={{ fontSize: '12px', fontWeight: '900', color: isSelected ? 'var(--primary-color)' : 'var(--text-main)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '4px' }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Monitor size={12} color={isSelected ? 'var(--primary-color)' : 'var(--text-muted)'} /> {getCardTitle(st, aggregationMode)}
                      </span>
          ...
          {paginatedNics.map(nic => {
            ...
            return (
              <tr key={nic.id} style={{ borderBottom: '1px solid var(--table-border)', backgroundColor: nic.status === 'SCRAPPED' ? 'rgba(239, 68, 68, 0.08)' : 'transparent' }}>
                ...
                <td style={{ ...tdStyle, whiteSpace: 'nowrap', ...hideCol('order_source') }}>{nic.custom_attributes?.order_source !== undefined ? (nic.custom_attributes?.order_source || '--') : (nic.custom_attributes?.order_date || '--')}</td>
                <td style={{ ...tdStyle, whiteSpace: 'nowrap', fontSize: '11px', color: nic.shipping_date ? '#8b5cf6' : 'var(--text-muted)', fontWeight: nic.shipping_date ? 700 : 'normal', ...hideCol('shipping_date') }}>
                  {nic.shipping_date ? new Date(nic.shipping_date).toLocaleDateString() : '--'}
                </td>
                <td style={{ ...tdStyle, ...hideCol('server') }}>
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={cardStyle}>
        ...
        {/* 硬體卡片聚合規則說明（三個列表共用同一份說明） */}
        <CardAggregationLegend unit="硬體" mode={aggregationMode} />
      </div>
