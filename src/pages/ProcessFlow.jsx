  ...
  const modulesData = useMemo(() => [
    ...
    },
    {
      ...
      businessRules: [
        ...
        '細項不勾選：範本裡定義的是欄位名稱（例如「OS」），由設備填寫實際內容（例如「RH9.6」），離開欄位即自動儲存。細項預設由每台設備各自挑選或新增；若每台都要填，可在範本頁勾選該細項前方的框，即與主要檢查功能一樣自動套用到該廠牌的所有設備。主要檢查功能以勾選表示檢查完成，設備列表狀態欄底下顯示的完成進度只計算主要檢查功能。細項是這台設備個別的內容、不是主要檢查功能底下的分支，因此在設備視窗與列印表上都獨立成「細項紀錄」一區（項目｜內容兩欄），不與檢查項目混在同一張表。',
