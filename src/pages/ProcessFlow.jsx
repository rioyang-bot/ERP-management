  ...
  const modulesData = useMemo(() => [
    ...
    },
    {
      ...
      subModules: [
        ...
        { name: '出機檢查表 (Pre-delivery Checklist)', path: '/checklist-templates', desc: '建立出機前檢查項目：主項目可綁定廠牌，底下分為整組帶入的「主要檢查功能」與由各設備自行挑選的「細項」；於設備列表逐台套用、勾選與列印' }
      ],
      ...
      businessRules: [
        ...
        '出機檢查表：於「報表中心 → 出機檢查表」建立主項目（可綁定廠牌）與其底下的「主要檢查功能」與「細項」兩組清單。設備列表的功能選單點選「出機檢查表」即可套用——主要檢查功能整組帶入，細項由每台設備各自挑選，因為每台要檢查的東西不盡相同；套用後在同一個視窗逐項勾選表示檢查完成，狀態欄底下會顯示完成進度。',
