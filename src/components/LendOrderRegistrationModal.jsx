...
const LendOrderRegistrationModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const [header, setHeader] = useState(() => {
    const saved = localStorage.getItem('lend_draft_header');
  ...
  const [outboundItems, setOutboundItems] = useState(() => {
    const saved = localStorage.getItem('lend_draft_items');
    return saved ? JSON.parse(saved) : [];
  });
        ...
        if (res.success && res.rows.length > 0) {
          ...
          setOutboundItems(prev => {
            ...
            if (filteredNewItems.length > 0) {
              ...
              alert(`專案 [${selectedProject}] 的所有在庫資產已在借用清單中。`);
            }
            return prev;
          });
  ...
  const handleClearDraft = () => {
    if (!window.confirm('確定要清空當前借用單的填寫內容嗎？')) return;
    setOutboundItems([]);
    setHeader({
      customer: '',
      customerId: '',
    ...
    localStorage.removeItem('lend_draft_items');
    localStorage.removeItem('lend_draft_header');
  };

  const handleSubmit = async () => {
    if (!header.customer) return alert('請選擇借貸對象 (客戶)');
    if (outboundItems.length === 0) return alert('借用清單中無任何品項');
    ...
    try {
      ...
      const createdDnInfo = {
        ...
        status: 'PENDING',
        request_type: 'LEND',
        creator_name: authUser?.full_name || 'Admin'
      };
      const createdItemsSnapshot = outboundItems.map(i => ({
        ...i,
      ...
      if (onSuccess) {
        onSuccess(createdDnInfo, createdItemsSnapshot);
      }
    } catch (err) {
          ...
          <div className="dn-content-layout">
            ...
            <div className="dn-left-panel">
                        ...
                        onChange={e => {
                          ...
                          if (customerData) {
                            ...
                            setHeader(prev => ({
                              ...
                              location: customerData.address || prev.location || ''
                            }));
                          } else {
                            setHeader(prev => ({
                              ...prev, 
                  ...
                  <div className="dn-field">
                    ...
                    <div className="input-with-icon">
                      ...
                      <input
                        ...
                        onChange={e => setHeader(prev => ({ ...prev, expected_return_date: e.target.value }))}
                      />
                    </div>
                  </div>
              ...
              <div className="dn-card">
                ...
                <div className="consumables-scroll-box" style={{ maxHeight: '180px' }}>
                  {!hasCsmFilter ? (
            ...
            <div className="dn-right-panel">
              <div className="dn-card dn-items-card">
                ...
                <div className="dn-items-container" style={{ minHeight: '360px', maxHeight: '480px' }}>
                  {outboundItems.length === 0 ? (
                      ...
                      <div key={item.tempId || idx} className="outbound-item-row">
                        ...
                        <div className="item-row-right">
                          ...
                          <input 
                            ...
                            value={item.purpose ?? ''}
                            onChange={e => updatePurpose(item.tempId, e.target.value)}
...
export default LendOrderRegistrationModal;
