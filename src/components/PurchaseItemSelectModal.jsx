...
const PurchaseItemSelectModal = ({
  ...
  const handleToggleSelectAll = () => {
    const allCurrentFilteredChecked = filteredItems.length > 0 && filteredItems.every((i) => checkedIds[i.id]);
    const nextChecked = { ...checkedIds };
    const nextQuantities = { ...quantities };

    filteredItems.forEach((i) => {
      nextChecked[i.id] = !allCurrentFilteredChecked;
      if (!allCurrentFilteredChecked && !nextQuantities[i.id]) {
      ...
      >
        ...
        <div style={{
          ...
          <div>
            <h3 style={{ margin: 0, fontSize: '18px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '8px' }}>
              <ShoppingCart size={20} color="var(--primary-color)" /> 挑選採購品項 (Select Purchase Items)
            </h3>
            <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: 'var(--text-muted)' }}>
              可切換類別標籤或輸入關鍵字搜尋，支援勾選單筆或多筆品項批次加入採購單。
            </p>
          </div>
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              <select
                ...
                data-testid="purchase-brand-select"
              >
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '12px', marginBottom: '16px' }}>
              <thead>
                <tr style={{ backgroundColor: 'var(--bg-surface-subtle)', textAlign: 'left', borderBottom: '1px solid var(--border-color)' }}>
                  ...
                  <th style={{ padding: '10px 10px', fontSize: '12px', fontWeight: 800, width: '90px', textAlign: 'center' }}>採購數量</th>
                  <th style={{ padding: '10px 10px', fontSize: '12px', fontWeight: 800, width: '65px', textAlign: 'center' }}>加入</th>
                </tr>
              </thead>
              <tbody>
                {filteredItems.map((item) => {
                  ...
                  return (
                    <tr
                      ...
                      data-testid={`purchase-item-row-${item.id}`}
                    >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            >
              <ShoppingCart size={15} /> 批次加入採購單 ({checkedCount})
            </button>
          </div>
        </div>
      </div>
...
export default PurchaseItemSelectModal;
